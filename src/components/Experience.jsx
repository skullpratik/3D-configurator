...
export const Experience = forwardRef(({ lighting = "photo_studio_01_4k.hdr", metalness = 1, roughness = 0.4, lightSettings = {} }, ref) => {
  ...
  }, [scene, gl, camera, threeScene, metalness, roughness]);

  useEffect(() => {
    if (!scene) return;
    console.log("All object names in GLTF scene:");
    scene.traverse((child) => {
      if (child.name) {
  ...
  return (
    <Suspense fallback={null}>
      ...
      <Environment key={lighting} files={`/${lighting}`} background={false} intensity={1.2} />
